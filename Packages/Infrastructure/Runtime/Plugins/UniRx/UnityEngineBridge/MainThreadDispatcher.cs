...
#pragma warning restore CS0618
        ...
        {
            if (Instance is MonoBehaviour mb) mb.StopCoroutine(coroutine);
        }
        ...
        public static IObservable<Unit> UpdateAsObservable() 
            => Instance.update ?? (Instance.update = new Subject<Unit>());

        Subject<Unit> lateUpdate;

        void LateUpdate() => lateUpdate?.OnNext(Unit.Default);

        public static IObservable<Unit> LateUpdateAsObservable() 
            => Instance.lateUpdate ?? (Instance.lateUpdate = new Subject<Unit>());

        Subject<bool> onApplicationFocus;

        void OnApplicationFocus(bool focus) => onApplicationFocus?.OnNext(focus);

        public static IObservable<bool> OnApplicationFocusAsObservable() 
            => Instance.onApplicationFocus ?? (Instance.onApplicationFocus = new Subject<bool>());

        Subject<bool> onApplicationPause;

        void OnApplicationPause(bool pause) => onApplicationPause?.OnNext(pause);

        public static IObservable<bool> OnApplicationPauseAsObservable() 
            => Instance.onApplicationPause ?? (Instance.onApplicationPause = new Subject<bool>());
        ...
        public static IObservable<Unit> OnApplicationQuitAsObservable() 
            => Instance.onApplicationQuit ?? (Instance.onApplicationQuit = new Subject<Unit>());
    }
}