...
namespace Origine.ObjectPool
{
    internal sealed partial class ObjectPoolManager : GameModule, IObjectPoolManager
    {
        ...
        private sealed class ObjectPool<T> : ObjectPoolBase, IObjectPool<T> where T : PoolableObject
        {
            ...
            public void Unspawn(object target)
            {
                ...
                {
                    throw new GameException(Utility.Text.Format("Can not find target in object pool '{0}', target type is '{1}', target value is '{2}'.", new TypeNamePair(typeof(T), Name).ToString(), target.GetType().FullName, target.ToString()));
                }
            }
            ...
            {
                m_AutoReleaseTime += deltaTime;
                if (AutoReleaseInterval == null || m_AutoReleaseTime < m_AutoReleaseInterval)
                    return;
            ...
            private void ReleaseObject(T obj)
            {
                ...
                if (internalObject == null)
                {
                    throw new GameException("Can not release object which is not found.");
                }
